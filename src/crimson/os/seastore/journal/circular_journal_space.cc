    ...
    rbm_abs_addr offset, bufferlist &bl)
{
  ...
  DEBUG(
    ...
    offset,
    length);
  return device->writev(offset, bl, 1 + (seastar::this_shard_id()%6) /* stream */
  ).handle_error(
...
CircularJournalSpace::write_header()
{
  ...
  return device->write(device->get_shard_journal_start(), std::move(bp), 1 + (seastar::this_shard_id()%6) /* stream */
  ).handle_error(
    submit_ertr::pass_further{},
    crimson::ct_error::assert_all{ "Invalid error device->write" }
  );
